...
export const PlayerControls = (
    ...
    useFrame((_state, delta) => {
        if (!playerRef.current || !environmentRef.current) return;
        ...
        const rayDirection = idealCameraPosition.clone().sub(rayFromPlayer)
            .normalize();
        ...
        const intersects = raycaster.intersectObjects(
            environmentRef.current.children,
            true,
        );

        const firstHit = intersects[0];
        if (firstHit && firstHit.distance < rayLength) {
            finalCameraPosition.copy(rayFromPlayer).add(
                rayDirection.multiplyScalar(firstHit.distance * 0.95),
            );
        } else {
